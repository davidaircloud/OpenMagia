...
async function renderSettings() {
  ...
  const videoInstalled=(management.installations||[]).filter(item=>item.available).map(item=>'<div class="managedModelRow"><span><b>'+esc(item.name||'MiniMax H3')+'</b><small>Video generation</small></span><em>'+(item.active?'In use':'Installed')+'</em>'+(item.managed?'<button class="btn ghost danger" data-uninstall-model="'+esc(item.id)+'">Uninstall</button>':'<small>External</small>')+'</div>').join('');
  const musicInstalled=music.local?.installed?'<div class="managedModelRow"><span><b>YuE 2</b><small>Music generation</small></span><em>'+(music.ready?'In use':'Installed')+'</em></div>':'';
  const installed=videoInstalled+musicInstalled||'<p class="modelEmpty">No generation models are installed. Choose one from Available.</p>';
