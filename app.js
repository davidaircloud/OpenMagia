...
async function renderSettings() {
  ...
  const installed=(management.installations||[]).filter(item=>item.available).map(item=>'<div class="managedModelRow"><span><b>'+esc(item.name||'Generation model')+'</b><small>'+(item.backend_id==='yue2'?'Music':'Video')+' generation</small></span><em>'+(item.active?'In use':'Installed')+'</em>'+(item.managed?'<button class="btn ghost danger" data-uninstall-model="'+esc(item.id)+'">Uninstall</button>':'<small>External</small>')+'</div>').join('')||'<p class="modelEmpty">No generation models are installed. Choose one from Available.</p>';
