...
async function openModelUninstall(installationId,knownManagement=null){
  ...
  const alternatives=(management.installations||[]).filter(item=>item.id!==installationId&&item.available&&(item.backend_id==='yue2')===(installation.backend_id==='yue2'));
  ...
  const medium=installation.backend_id==='yue2'?'Music':'Video';
...
async function renderSettings() {
  ...
  const formatterInstalled=e.formatter?'<div class="managedModelRow"><span><b>Qwen2.5 7B Instruct</b><small>Prompt refinement · 4.7 GB</small></span><em>In use</em><small>OpenMagia</small></div>':'';
  const generationModels=(management.installations||[]).filter(item=>item.available).map(item=>'<div class="managedModelRow"><span><b>'+esc(item.name||'Generation model')+'</b><small>'+(item.backend_id==='yue2'?'Music':'Video')+' generation</small></span><em>'+(item.active?'In use':'Installed')+'</em>'+(item.managed?'<button class="btn ghost danger" data-uninstall-model="'+esc(item.id)+'">Uninstall</button>':'<small>External</small>')+'</div>').join('');
  const installed=formatterInstalled+generationModels||'<p class="modelEmpty">No models are installed. Choose one from Available or Add-ons.</p>';
