...
async function renderSettings() {
  ...
  const generationModels=(management.installations||[]).filter(item=>item.available).map(item=>{const role=item.backend_id==='yue2'?'Music generation':item.backend_id==='qwen2.5-7b'?'Prompt refinement':'Video generation',component=item.backend_id==='yue2'?'yue':item.backend_id==='qwen2.5-7b'?'formatter':'h3',job=(e.model_installs||{})[component]||{},updating=job.status==='running';return '<div class="managedModelRow"><span><b>'+esc(item.name||'Model')+'</b><small>'+role+'</small></span><em>'+(item.active?'In use':'Installed')+'</em>'+(item.managed?'<button class="btn ghost" data-update-model="'+component+'" '+(updating?'disabled':'')+'>'+(updating?'Checking…':'Check for updates')+'</button><button class="btn ghost danger" data-uninstall-model="'+esc(item.id)+'">Uninstall</button>':'<small>External</small>')+'</div>';}).join('');
  ...
  $$('[data-update-model]',body).forEach(button=>button.addEventListener('click',async()=>{const component=button.dataset.updateModel;button.disabled=true;try{await api('/api/models/install',{method:'POST',body:{component,accepted_license:true,update:true}});toast('Checking the installed model source for updates…','ok');await refresh();renderSettings();}catch(error){button.disabled=false;toast(error.message,'err');}}));
